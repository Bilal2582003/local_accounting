...
if(isset($_POST['id'])){
    ...
    include "../Model/connection.php";
        $query="SELECT p_khata.*,party.name as worker_name from p_khata join party on p_khata.party_id = party.id where p_khata.id = $id  order by receipt_date desc";    
    ...
    if(mysqli_num_rows($res) > 0){
        while($row=mysqli_fetch_assoc($res)){
            ...
            $workerData .=' <option selected value="'.$row["party_id"].'">'.$row["worker_name"].'</option>';
            $WorkerQuery="SELECT * from party where id != '$worker_id'";
