...
// function getDataAndCalculateSums($id) {
    ...
    $query = "SELECT p_khata.*,party.name as name FROM p_khata join party on p_khata.party_id = party.id WHERE party.id = '$id'  order by receipt_date desc";
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        ...
        // if($row['receipt_date']){
            $Rdate = !empty($row['receipt_date']) ? date("Y-M-d (D)", strtotime($row['receipt_date'])) : '-';
            // $Rdate = date("Y-M-d (D)");
        // }
        // if($row['created_at']){
